...
detection_col = headers.index('检测对象') + 1
...
if success:
    ...
    for device_type in excel_device_types.keys():
        ...
        print(f"     参数列表: {[p['name'] for p in config['params']]}")
        
else:
    print("❌ 配置更新失败")
    sys.exit(1)

print("\n🎉 霍尼韦尔传感器自动配置更新完成（包含检测对象）！")
