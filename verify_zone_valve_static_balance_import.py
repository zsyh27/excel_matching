...
with db_manager.session_scope() as session:
    ...
    total_devices = 0
    for device_type in zone_valve_device_types:
    ...
    for device_type in zone_valve_device_types:
        ...
        if device:
            ...
            if device.key_params:
                print(f"  参数列表（前5个）: {', '.join(list(device.key_params.keys())[:5])}...")
