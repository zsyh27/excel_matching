...
- [ ] 1. 清理前端特征权重配置相关代码
  - [ ] 1.1 从 `menuStructure.js` 中移除 `feature-weights` 菜单项
    - 删除 `pre-entry` 阶段 `items` 数组中 `id: 'feature-weights'` 的对象
    - _需求: 1.1, 1.2_

  - [ ] 1.2 从 `ConfigManagementView.vue` 中移除 FeatureWeightEditor 的引用
  ...
  - [ ] 1.3 从 `ConfigEditorContainer.vue` 中移除 FeatureWeightEditor 的引用
    - 删除导入语句、组件注册和映射条目
    - _需求: 1.3_

  - [ ] 1.4 从 `ConfigManagement/index.js` 中移除 FeatureWeightEditor 的导出
    - 删除 `export { default as FeatureWeightEditor } from './FeatureWeightEditor.vue'`
    - _需求: 1.3_

  - [ ]* 1.5 删除 `FeatureWeightEditor.vue` 文件
    - 确认无其他文件引用后删除该文件
    - _需求: 1.3_

- [ ] 2. 清理前端设备列表中的规则相关展示
  - [ ] 2.1 修改 `DeviceList.vue`，移除规则相关列和功能，替换为关键参数展示
...
- [ ] 3. 清理前端设备详情和数据一致性检查组件
  ...
  - [ ] 3.2 从 `frontend/src/api/database.js` 中移除规则相关 API 函数
  ...
  - [ ]* 3.3 删除 `ConsistencyCheck.vue` 文件
    - 确认无其他文件引用后删除该文件
    - _需求: 2.6_
...
- [ ] 5. 清理后端规则相关 API 端点
  - [ ] 5.1 从 `app.py` 中删除规则基础 CRUD API 端点
  ...
  - [ ] 5.2 从 `app.py` 中删除规则生成和重新生成 API 端点
  ...
  - [ ] 5.3 从 `app.py` 中删除所有 [DEPRECATED] 规则管理 API 端点
  ...
  - [ ]* 5.5 为已移除的规则 API 端点编写属性测试
    - 验证所有已移除的规则 API 端点返回 404
    - **Property 1: 已移除的规则 API 端点返回 404**
    - **Validates: Requirements 4.1, 4.2, 4.3**

- [ ] 6. 清理 app.py 中旧系统初始化和业务逻辑代码
  - [ ] 6.1 从 `app.py` 初始化代码中移除旧系统组件
  ...
  - [ ] 6.4 从 Excel 批量导入 API 中移除规则生成逻辑
...
- [ ] 8. 清理数据库 feature_weight_config 配置和 rules 表
  - [ ] 8.1 创建数据库清理脚本并执行
...
- [ ] 10. 更新 steering 文档
  - [ ] 10.1 更新 `.kiro/steering/intelligent-extraction-system-guide.md`
    - 移除对旧规则系统的引用（如"规则生成"、"RuleGenerator"等描述）
    - 确保文档准确反映当前五步流程架构
    - _需求: 8.1, 8.3_

  - [ ] 10.2 更新 `.kiro/steering/device-input-guide.md`
