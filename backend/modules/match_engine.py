...
class MatchEngine:
    ...
    def __init__(self, rules: List, devices: Dict, config: Dict):
        """
        初始化匹配引擎
        ...
        self.config = config
        self.default_match_threshold = config.get('global_config', {}).get('default_match_threshold', 2.0)
    ...
    def match(self, features: List[str]) -> MatchResult:
        ...
        # 如果有候选规则，选择最佳匹配
        if candidates:
            return self.select_best_match(candidates)
